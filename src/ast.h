...
struct Attr:Expr{E a;std::string n;Attr(E x,std::string s):a(std::move(x)),n(std::move(s)){}};
...
struct Function:Stmt{std::string name;std::vector<std::string> params;std::vector<E> defaults;std::vector<S> body;bool method=false;std::string owner;};
struct Class:Stmt{std::string name,base;std::vector<S> body;};
