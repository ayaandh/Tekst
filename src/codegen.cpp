...
void Codegen::emitBlock(const std::vector<S>&v){for(auto&s:v)emitStmt(s.get());}

void Codegen::emitFunction(Function*f){
 slots.clear();types.clear();nextId=0;nextBlock=0;body.str("");body.clear();currentFn=f->name;currentOwner=f->owner;
...
std::string Codegen::generate(const Program&p){
 ...
 // collect import aliases before emitting declarations
 for(auto&s:p.body){
 ...
 // collect declarations before emitting functions
 for(auto&s:p.body){
 ...
 slots.clear();types.clear();nextId=0;nextBlock=0;body.str("");body.clear();currentFn="main";currentOwner="";
 emitBlock(p.body);
 ir<<globals.str();
 ir<<"define i32 @main() {\nentry:\n"<<body.str()<<"  ret i32 0\n}\n";return ir.str();
}
